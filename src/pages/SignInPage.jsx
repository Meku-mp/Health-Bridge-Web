...
const SignInPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user) {
        navigate('/profile'); // Redirect to profile after login (adjust path as needed)
      }
    } catch (error) {
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex flex-col items-center justify-center w-2/3 px-8 max-xl:w-1/2 md:px-20 max-md:w-full">
        ...
        <p className="mt-6 text-[#101828] text-[14px]">Don't have an account?{' '}<button onClick={() => navigate('/signup')} className="text-[#0E1680] hover:text-[#090b22]  font-semibold text-[14px] ">Create Account</button></p>
      </div>
      {/* Right Image Section */}
